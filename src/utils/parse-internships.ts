...
export function parseInternshipData(content: string): Job[] {
  ...
  const target = tables.find((t) => {
    const ths = Array.from(t.querySelectorAll('thead th')).map((th) =>
      norm(th.textContent).toLowerCase()
    );
    return ['company', 'role', 'location', 'application', 'age'].every((h) =>
      ths.includes(h)
    );
  });
  if (!target) return [];
  ...
  for (const row of rows) {
    ...
    let company = norm(
      tds[0].querySelector('a')?.textContent || tds[0].textContent
    );
    ...
    const anchors = Array.from(
      tds[3].querySelectorAll<HTMLAnchorElement>('a[href]')
    );
    const apply = anchors.find((a) =>
      /apply|greenhouse|lever|workday|myworkdayjobs|careers|jobs|boards\.greenhouse/i.test(
        a.href
      )
    );
    const simplify = anchors.find((a) => /simplify\.jobs/i.test(a.href));
    const link = norm(
      (apply?.href || simplify?.href || anchors[0]?.href) ?? ''
    );
